...
-- // UPDATED: INVENTORY FLICKER FIX APPLIED //
    ...
    while true do
        if inventoryEnabled and not STEPS_IN_PROGRESS then
            ...
            if plrHud and mainGui then
                -- Forza la visibilità dell'inventario ON, senza cliccare il bottone
                local invFrame = mainGui:FindFirstChild("INVENTORY")
                if invFrame then
                    invFrame.Visible = true -- Mantiene la finestra aperta senza lampeggiare
                end

                -- Logica di Spostamento / Storage degli Item
                local found = false
                pcall(function()
                    if invFrame and invFrame:FindFirstChild("BackpackFrame") then
            ...
            -- Opzionale: Se disattivi, chiudi la finestra (rimuovi il -- se vuoi che si chiuda)
            -- local mainGui = LocalPlayer.PlayerGui:FindFirstChild("MainGui")
            -- if mainGui and mainGui:FindFirstChild("INVENTORY") then mainGui.INVENTORY.Visible = false end
            task.wait(1)
        end
    end
end)
...
if autoLoadConfigName ~= "" and savedConfigs[autoLoadConfigName] then
    ...
    -- Inizializza asincronamente per dare tempo a Rayfield
    task.spawn(function()
        task.wait(1.5) 
        updateAllUI()
